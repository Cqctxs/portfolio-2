      ...
      if (animationFrameId === null) {
        animationFrameId = requestAnimationFrame(() => {
          ...
          if (isDragging) {
            const newX = Math.max(
              0,
              Math.min(event.clientX - dragOffset.current.x, window.innerWidth - 100)
            );
            const newY = Math.max(
              0,
              Math.min(event.clientY - dragOffset.current.y, window.innerHeight - 100)
            );
            setDragPreview((prev) => {
              if (!prev) return null;
              if (prev.x === newX && prev.y === newY) return prev;
              return { x: newX, y: newY, width: prev.width, height: prev.height };
            });
          }
          ...
          if (isResizing) {
            ...
            const hasE = resizeDirection.indexOf("e") !== -1;
            const hasW = resizeDirection.indexOf("w") !== -1;
            const hasS = resizeDirection.indexOf("s") !== -1;
            const hasN = resizeDirection.indexOf("n") !== -1;
